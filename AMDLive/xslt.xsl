	...
	<xsl:variable name="processeddata">
		...
		<xsl:for-each select="//dataset[@id=$id]/*">
			<entry>
				...
				<xsl:if test="$id='sess'"><client><xsl:value-of select="client" /></client><ss><xsl:value-of select="ss" /></ss></xsl:if>
		...
		<totals>
			<sessions><xsl:value-of select="format-number(sum(//dataset[@id=$id]/*/sessions), '###,##0')"/></sessions>
	  ...
	  <table id="{$id}" class="table-autosort:{$sortcol} table-autopage:{$pagelen} table-page-number:{$id}page table-page-count:{$id}pages">
		<thead>
			<tr>
				...
				<xsl:if test="$id='sess'"><th class="left big table-sortable:ignorecase table-sortable filterable">Client</th><th class="left big table-sortable:ignorecase table-sortable filterable">Software Service</th></xsl:if>
			...
			<tr>
				...
				<xsl:if test="$id='sess'"><th class="left"><input name="{$id}clifilter" size="20" title="RegEx filter" onkeyup="Table.filter(this,this)"/></th><th class="left"><input name="{$id}ssfilter" size="20" title="RegEx filter" onkeyup="Table.filter(this,this)"/></th></xsl:if>
		...
		<xsl:for-each select="exsl:node-set($processeddata)/data/entry">
			<tr>
				...
				<xsl:if test="$id='sess'"><td class="left"><xsl:value-of select="client"/></td><td class="left"><xsl:value-of select="ss"/></td></xsl:if>
		...
		<tfoot>
			<tr class="total">
				...
				<xsl:if test="$id='sess'"><td class="left"></td><td class="left"></td></xsl:if>
